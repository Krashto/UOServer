	...
	public class SeismeSpell : Spell
	{
        ...
        public override void OnCast()
        {
            ...
            if (SpellHelper.CheckTown(Caster, Caster) && CheckSequence())
            {
                ...
                if (map != null)
                {
                    foreach (Mobile m in Caster.GetMobilesInRange((int)SpellHelper.AdjustValue(Caster, 1 + Caster.Skills[SkillName.Magery].Value / 15, NAptitude.Destruction)))
                    {
                ...
                for (int i = 0; i < m_target.Count; ++i)
                {
                    ...
                    if (Caster.CanSee(targ))
                    {
                        ...
                        double damage = GetNewAosDamage(targ, 15, 4, 6, true);
