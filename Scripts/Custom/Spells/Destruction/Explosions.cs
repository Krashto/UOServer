...
namespace Server.Spells
{
	public class ExplosionsSpell : Spell
	{
		...
		public ExplosionsSpell( Mobile caster, Item scroll ) : base( caster, scroll, m_Info )
		{
		}
        ...
        public void Target(IPoint3D p)
        {
            ...
            else if (CheckSequence())
            {
                ...
                if (map != null)
                {
                    IPooledEnumerable eable = map.GetMobilesInRange(new Point3D(p), (int)SpellHelper.AdjustValue(Caster, 2 + Caster.Skills[CastSkill].Value / 50, NAptitude.Destruction));
                ...
                if (targets.Count > 0)
                {
                    for (int i = 0; i < targets.Count; ++i)
                    {
                        ...
                        if (Caster.CanSee(m))
                        {
                            ...
                            double damage = GetNewAosDamage(m, 15, 1, 5, true);
