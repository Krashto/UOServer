...
namespace Server.Spells
{
	public class SpiritualiteSpell : ReligiousSpell
	{
		...
		{
			if ( !(m is BaseCreature || m is CustomPlayerMobile) )
			{
            ...
            else if (CheckHSequence(m))
            {
                ...
                TimeSpan delay = TimeSpan.FromSeconds(8 - Caster.Skills[CastSkill].Value / 20); //8 - 3

                int amount = (int)(2 + Caster.Skills[CastSkill].Base / 10);
                amount = (int)SpellHelper.AdjustValue(m, amount, NAptitude.MagieAncestrale);
