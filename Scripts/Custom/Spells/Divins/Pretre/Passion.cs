    ...
    public class PassionSpell : ReligiousSpell
    {
        ...
        public void Target(Mobile m)
        {
            ...
            else if (CheckSequence())
            {
                ...
                TimeSpan duration = GetDurationForSpell(1.0);
                ...
                if (val > 0)
                {
                    m_PassionTable[m] = -1 * val;
                    m_PassionTable[Caster] = val;
        ...
        public class PassionTimer : Timer
        {
            ...
            public PassionTimer(Mobile target, DateTime end) : base(TimeSpan.Zero, TimeSpan.FromSeconds(2))
            {
                m_target = target;
                endtime = end;
            ...
            {
                if ((DateTime.Now >= endtime && m_PassionTable.Contains(m_target)) || m_target == null || m_target.Deleted || !m_target.Alive)
                {
                    m_PassionTable.Remove(m_target);
                    m_Timers.Remove(m_target);

                    //Si on a un caster ou un target qui possède la modification, on le retire aussi.
                    if (m_LinkTable.Contains(m_target))
                    {
                        StopTimer((Mobile)m_LinkTable[m_target]);
                        m_LinkTable.Remove(m_target);
                    }
