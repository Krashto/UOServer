	...
	public class TrouDeVerSpell : Spell
	{
		...
		public void Effect(Point3D loc, Map map, bool checkMulti)
		{
			...
			else if (CheckSequence())
			{
				...
				var firstGate = new InternalItem(loc, map, duration);
				firstGate.MoveToWorld(Caster.Location, Caster.Map);

				Effects.PlaySound(loc, map, 0x20E);

				var secondGate = new InternalItem(Caster.Location, Caster.Map, duration);
		...
		private class InternalItem : Moongate
		{
			...
			public InternalItem(Point3D target, Map map, double duration) : base(target, map)
			{
				...
				var t = new InternalTimer(this, m_Duration);
				t.Start();
			}
