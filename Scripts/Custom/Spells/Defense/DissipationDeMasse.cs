...
namespace Server.Spells
{
	public class DissipationDeMasseSpell : Spell
	{
        ...
        public override NAptitude[] RequiredAptitude { get { return new NAptitude[] { NAptitude.Defense }; } }
		...
		public void Target( IPoint3D p )
		{
			...
			else if ( CheckSequence() )
			{
				...
				if ( map != null )
				{
					...
					{
                        if (m is BaseCreature && ((BaseCreature)m).Summoned && !((BaseCreature)m).IsAnimatedDead && Caster.CanBeHarmful(m, false) && !CustomPlayerMobile.IsInEquipe(Caster, m))
							targets.Add( m );
					}
				...
				for ( int i = 0; i < targets.Count; ++i )
				{
					...
					double dispelChance = (50.0 + ((100 * (Caster.Skills.Magery.Value - bc.DispelDifficulty)) / (bc.DispelFocus * 2))) / 120;
