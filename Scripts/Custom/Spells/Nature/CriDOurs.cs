...
namespace Server.Spells
{
	public class CriDOursSpell : Spell
	{
        ...
        public override NAptitude[] RequiredAptitude { get { return new NAptitude[] { NAptitude.Nature }; } }
        ...
        public override void OnCast()
        {
            if (CheckSequence())
            {
                ...
                if (map != null)
                {
                    IPooledEnumerable eable = map.GetMobilesInRange(new Point3D(Caster.Location), (int)SpellHelper.AdjustValue(Caster, 1 + Caster.Skills[SkillName.Magery].Value / 20, NAptitude.Nature));
                ...
                if (targets.Count > 0)
                {
                    for (int i = 0; i < targets.Count; ++i)
                    {
                        ...
                        {
                            new CriDOursTimer((CustomPlayerMobile)m, endtime).Start();
                        }
        ...
        private class CriDOursTimer : Timer
        {
            ...
            public CriDOursTimer(CustomPlayerMobile target, DateTime endtime)
