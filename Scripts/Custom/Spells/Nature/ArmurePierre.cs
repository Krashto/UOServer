...
namespace Server.Spells
{
	public class ArmurePierreSpell : Spell
	{
		...
		private class ArmurePierreTarget : Target
		{
			...
			{
                if (targeted is CustomPlayerMobile && m_spell.CheckBSequence((Mobile)targeted))
				{
                    CustomPlayerMobile targ = (CustomPlayerMobile)targeted;
        ...
        {
            private CustomPlayerMobile player;

            public InternalTimer(CustomPlayerMobile pm, TimeSpan duration)
                : base(duration)
            {
                player = pm;
