    ...
    abstract class NewGuildBaseGump : BaseProjectMGump
    {
        ...
        protected void RankUpMobile_OnTarget(Mobile from, object targeted)
        {
            if (targeted is CustomPlayerMobile)
            {
                ...
                from.SendMessage("Son rang est maintenant : " + m_Guild.GetTitleByRank(m_Guild.GeCustomPlayerMobileRank((CustomPlayerMobile)targeted)));
            }
            else
            {
                from.SendMessage("Vous devez choisir un joueur");
    ...
    class NewGuildGump : NewGuildBaseGump
    {
            ...
            : base(from, guild)
        {
            ...
            if (m_Guild != null)
            {
                ...
                if (m_From.AccessLevel >= AccessLevel.GameMaster || m_Guild.GeCustomPlayerMobileRank(m_From) >= (GuildRecruter._RANKMAX - 1))
                {
                ...
                else
                {
                    ...
                    if (m_Guild.GeCustomPlayerMobileRank(m_From) == -1)
                    {
                    ...
                    else
                    {
                        ...
                        line++;
                        AddHtml(x, y + (space * line++), 450, 20, "<BASEFONT COLOR=#FFFFFF>Votre titre est " + m_Guild.GetTitleByRank(m_Guild.GeCustomPlayerMobileRank(m_From)) + " (" + "Rang " + m_Guild.GeCustomPlayerMobileRank(m_From) + ")</BASEFONT>", false, false);
                        AddHtml(x, y + (space * line++), 450, 20, "<BASEFONT COLOR=#FFFFFF>Votre rente est de " + m_Guild.GetSalaryByRank(m_Guild.GeCustomPlayerMobileRank(m_From)) + " pièces d'or par semaine.</BASEFONT>", false, false);
