...
{
	public class MiniCoffeeSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new MiniCoffeeSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class MiniCoffeeSeedling : BaseCrop
	{
		...
		public static void init( MiniCoffeeSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(MiniCoffeeCrop), plant.Sower );
	...
	public class MiniCoffeeCrop : BaseCrop
	{
		...
		public static void init ( MiniCoffeeCrop plant, bool full )
		{
		...
		{
			private MiniCoffeeCrop i_plant;
			public CropTimer( MiniCoffeeCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						CoffeeBean fruit = new CoffeeBean( Utility.Random( m_yield +2 ) );
