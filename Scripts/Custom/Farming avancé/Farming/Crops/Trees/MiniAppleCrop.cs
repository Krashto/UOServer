...
{
	public class MiniAppleSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new MiniAppleSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class MiniAppleSeedling : BaseCrop
	{
		...
		public static void init( MiniAppleSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(MiniAppleCrop), plant.Sower );
	...
	public class MiniAppleCrop : BaseCrop
	{
		...
		public static void init ( MiniAppleCrop plant, bool full )
		{
		...
		{
			private MiniAppleCrop i_plant;
			public CropTimer( MiniAppleCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Apple fruit = new Apple( Utility.Random( m_yield +2 ) );
