...
{
	public class MiniCherrySeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new MiniCherrySeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class MiniCherrySeedling : BaseCrop
	{
		...
		public static void init( MiniCherrySeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(MiniCherryCrop), plant.Sower );
	...
	public class MiniCherryCrop : BaseCrop
	{
		...
		public static void init ( MiniCherryCrop plant, bool full )
		{
		...
		{
			private MiniCherryCrop i_plant;
			public CropTimer( MiniCherryCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Cherry fruit = new Cherry( Utility.Random( m_yield +2 ) );
