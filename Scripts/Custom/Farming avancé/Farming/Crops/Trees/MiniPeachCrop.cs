...
{
	public class MiniPeachSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new MiniPeachSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class MiniPeachSeedling : BaseCrop
	{
		...
		public static void init( MiniPeachSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(MiniPeachCrop), plant.Sower );
	...
	public class MiniPeachCrop : BaseCrop
	{
		...
		public static void init ( MiniPeachCrop plant, bool full )
		{
		...
		{
			private MiniPeachCrop i_plant;
			public CropTimer( MiniPeachCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Peach fruit = new Peach( Utility.Random( m_yield +2 ) );
