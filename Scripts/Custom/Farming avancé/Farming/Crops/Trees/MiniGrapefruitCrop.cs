...
{
	public class MiniGrapefruitSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new MiniGrapefruitSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class MiniGrapefruitSeedling : BaseCrop
	{
		...
		public static void init( MiniGrapefruitSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(MiniGrapefruitCrop), plant.Sower );
	...
	public class MiniGrapefruitCrop : BaseCrop
	{
		...
		public static void init ( MiniGrapefruitCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						Grapefruit crop = new Grapefruit( pick );
		...
		{
			private MiniGrapefruitCrop i_plant;
			public CropTimer( MiniGrapefruitCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Grapefruit fruit = new Grapefruit( Utility.Random( m_yield +2 ) );
