...
{
	public class CoconutSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new CoconutSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class CoconutSeedling : BaseCrop
	{
		...
		public static void init( CoconutSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(CoconutCrop), plant.Sower );
	...
	public class CoconutCrop : BaseCrop
	{
		...
		public static void init ( CoconutCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xC95 );
			plant.FullGraphic = ( 0xC95 );
		...
		{
			private CoconutCrop i_plant;
			public CropTimer( CoconutCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Coconut fruit = new Coconut( Utility.Random( m_yield +2 ) );
