...
{
	public class MiniKiwiSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new MiniKiwiSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class MiniKiwiSeedling : BaseCrop
	{
		...
		public static void init( MiniKiwiSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(MiniKiwiCrop), plant.Sower );
	...
	public class MiniKiwiCrop : BaseCrop
	{
		...
		public static void init ( MiniKiwiCrop plant, bool full )
		{
		...
		{
			private MiniKiwiCrop i_plant;
			public CropTimer( MiniKiwiCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Kiwi fruit = new Kiwi( Utility.Random( m_yield +2 ) );
