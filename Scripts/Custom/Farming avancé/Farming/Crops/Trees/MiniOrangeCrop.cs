...
namespace Server.Items.Crops
{
	...
	public class MiniOrangeSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new MiniOrangeSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class MiniOrangeSeedling : BaseCrop
	{
		...
		public static void init( MiniOrangeSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(MiniOrangeCrop), plant.Sower );
	...
	public class MiniOrangeCrop : BaseCrop
	{
		...
		public static void init ( MiniOrangeCrop plant, bool full )
		{
		...
		{
			private MiniOrangeCrop i_plant;
			public CropTimer( MiniOrangeCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Orange fruit = new Orange( Utility.Random( m_yield +2 ) );
