...
namespace Server.Items.Crops
{
	...
	public class MiniLemonSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new MiniLemonSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class MiniLemonSeedling : BaseCrop
	{
		...
		public static void init( MiniLemonSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(MiniLemonCrop), plant.Sower );
	...
	public class MiniLemonCrop : BaseCrop
	{
		...
		public static void init ( MiniLemonCrop plant, bool full )
		{
		...
		{
			private MiniLemonCrop i_plant;
			public CropTimer( MiniLemonCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Lemon fruit = new Lemon( Utility.Random( m_yield +2 ) );
