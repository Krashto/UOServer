...
{
	public class MiniApricotSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new MiniApricotSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class MiniApricotSeedling : BaseCrop
	{
		...
		public static void init( MiniApricotSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(MiniApricotCrop), plant.Sower );
	...
	public class MiniApricotCrop : BaseCrop
	{
		...
		public static void init ( MiniApricotCrop plant, bool full )
		{
		...
		{
			private MiniApricotCrop i_plant;
			public CropTimer( MiniApricotCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Apricot fruit = new Apricot( Utility.Random( m_yield +2 ) );
