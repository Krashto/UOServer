...
{
	public class DateSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new DateSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class DateSeedling : BaseCrop
	{
		...
		public static void init( DateSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(DateCrop), plant.Sower );
	...
	public class DateCrop : BaseCrop
	{
		...
		public static void init ( DateCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xC96 );
			plant.FullGraphic = ( 0xC96 );
		...
		{
			private DateCrop i_plant;
			public CropTimer( DateCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Dates fruit = new Dates( Utility.Random( m_yield +2 ) );
