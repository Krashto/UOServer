...
namespace Server.Items.Crops
{
	...
	public class MiniLimeSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new MiniLimeSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class MiniLimeSeedling : BaseCrop
	{
		...
		public static void init( MiniLimeSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(MiniLimeCrop), plant.Sower );
	...
	public class MiniLimeCrop : BaseCrop
	{
		...
		public static void init ( MiniLimeCrop plant, bool full )
		{
		...
		{
			private MiniLimeCrop i_plant;
			public CropTimer( MiniLimeCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Lime fruit = new Lime( Utility.Random( m_yield +2 ) );
