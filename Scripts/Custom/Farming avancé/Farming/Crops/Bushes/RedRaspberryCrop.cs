...
{
	public class RedRaspberrySeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new RedRaspberrySeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class RedRaspberrySeedling : BaseCrop
	{
		...
		public static void init( RedRaspberrySeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(RedRaspberryCrop), plant.Sower );
	...
	public class RedRaspberryCrop : BaseCrop
	{
		...
		public static void init ( RedRaspberryCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						RedRaspberry crop = new RedRaspberry( pick );
		...
		{
			private RedRaspberryCrop i_plant;
			public CropTimer( RedRaspberryCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
