...
{
	public class BlackberrySeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new BlackberrySeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class BlackberrySeedling : BaseCrop
	{
		...
		public static void init( BlackberrySeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(BlackberryCrop), plant.Sower );
	...
	public class BlackberryCrop : BaseCrop
	{
		...
		public static void init ( BlackberryCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						Blackberry crop = new Blackberry( pick );
		...
		{
			private BlackberryCrop i_plant;
			public CropTimer( BlackberryCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Blackberry fruit = new Blackberry( Utility.Random( m_yield +2 ) );
