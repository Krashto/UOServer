...
{
	public class BlackRaspberrySeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new BlackRaspberrySeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class BlackRaspberrySeedling : BaseCrop
	{
		...
		public static void init( BlackRaspberrySeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(BlackRaspberryCrop), plant.Sower );
	...
	public class BlackRaspberryCrop : BaseCrop
	{
		...
		public static void init ( BlackRaspberryCrop plant, bool full )
		{
		...
		{
			private BlackRaspberryCrop i_plant;
			public CropTimer( BlackRaspberryCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						BlackRaspberry fruit = new BlackRaspberry( Utility.Random( m_yield +2 ) );
