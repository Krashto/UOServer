...
namespace Server.Items.Crops
{
	...
	public class BlueberrySeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new BlueberrySeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class BlueberrySeedling : BaseCrop
	{
		...
		public static void init( BlueberrySeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(BlueberryCrop), plant.Sower );
	...
	public class BlueberryCrop : BaseCrop
	{
		...
		public static void init ( BlueberryCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						Blueberry crop = new Blueberry( pick );
		...
		{
			private BlueberryCrop i_plant;
			public CropTimer( BlueberryCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
