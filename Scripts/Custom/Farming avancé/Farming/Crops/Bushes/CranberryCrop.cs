...
{
	public class CranberrySeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new CranberrySeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class CranberrySeedling : BaseCrop
	{
		...
		public static void init( CranberrySeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(CranberryCrop), plant.Sower );
	...
	public class CranberryCrop : BaseCrop
	{
		...
		public static void init ( CranberryCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						Cranberry crop = new Cranberry( pick );
		...
		{
			private CranberryCrop i_plant;
			public CropTimer( CranberryCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Cranberry fruit = new Cranberry( Utility.Random( m_yield +2 ) );
