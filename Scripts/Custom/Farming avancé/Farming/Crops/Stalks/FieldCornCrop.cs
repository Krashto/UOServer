...
{
	public class FieldCornSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new FieldCornSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class FieldCornSeedling : BaseCrop
	{
		...
		public static void init( FieldCornSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(FieldCornCrop), plant.Sower );
	...
	public class FieldCornCrop : BaseCrop
	{
		...
		public static void init ( FieldCornCrop plant, bool full )
		{
		...
		{
			private FieldCornCrop i_plant;
			public CropTimer( FieldCornCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						FieldCorn fruit = new FieldCorn( Utility.Random( m_yield +2 ) );
