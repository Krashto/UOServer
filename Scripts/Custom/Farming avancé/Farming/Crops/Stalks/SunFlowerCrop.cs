...
{
	public class SunFlowerSeed : BaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new SunFlowerSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class SunFlowerSeedling : BaseCrop
	{
		...
		public static void init( SunFlowerSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(SunFlowerCrop), plant.Sower );
	...
	public class SunFlowerCrop : BaseCrop
	{
		...
		public static void init ( SunFlowerCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xC85 );
			plant.FullGraphic = ( 0xC85 );
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						EdibleSun crop = new EdibleSun( pick );
		...
		{
			private SunFlowerCrop i_plant;
			public CropTimer( SunFlowerCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						EdibleSun fruit = new EdibleSun( Utility.Random( m_yield +2 ) );
