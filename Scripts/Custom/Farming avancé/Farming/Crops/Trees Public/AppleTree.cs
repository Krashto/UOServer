...
{
	public class AppleSapling : BaseCrop
	{
		...
		[Constructable]
		public AppleSapling() : base( Utility.RandomList ( 0xCE9, 0xCEA ) )
		{
			Movable = false;
		...
		public AppleSapling( Serial serial ) : base( serial ){}
	...
	public class AppleTree : BaseTree
	{
		...
		[Constructable]
		public AppleTree( Point3D pnt, Map map ) : base( Utility.RandomList( 0xD96, 0xD9A ) )
		{
			Movable = false;
			MoveToWorld( pnt, map );
		...
		public static void init ( AppleTree plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 2 ) )
				{
					...
					else
					{
						...
						m_yield -= pick;
						from.SendMessage( "You pick {0} apple{1}!", pick, ( pick == 1 ? "" : "s" ) );

						Apple crop = new Apple( pick );
		...
		{
			private AppleTree i_plant;

			public FruitTimer( AppleTree plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		{
			private AppleTree i_tree;
			private FruitTreeStump i_stump;
			private Mobile m_chopper;

			public StumpTimer( AppleTree Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
