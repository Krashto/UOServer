...
namespace Server.Gumps
{
	public class TestStoneGump : BaseProjectMGump
	{
		...
		public TestStoneGump(CustomPlayerMobile from) : base("Test", 700, 520, false)
		{
			...
			#region Experience
			AddSection(x - 10 + columnSpace * column, y + lineSpace * line++, 250, lineSpace * 5 - 1, "Expérience");
			line++;
			if (m_From.Experience.Niveau > 0)
				AddButton(x + 120 + columnSpace * column, y + lineSpace * line + 2, 5603, 5607, 1, GumpButtonType.Reply, 0);
			if (m_From.Experience.Niveau < 50)
			...
			line++;
			AddButtonHtml(x + 10 + columnSpace * column, y + lineSpace * line++, 10, "Pièces d'or", "#FFFFFF");
			AddButtonHtml(x + 10 + columnSpace * column, y + lineSpace * line++, 11, "Matériaux", "#FFFFFF");
			#endregion
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			if (info.ButtonID == 1)
			{
				m_From.Experience.Niveau--;
				m_From.Experience.Exp = Experience.GetRequiredExpByLevel(m_From.Experience.Niveau);
